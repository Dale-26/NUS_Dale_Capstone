...
The complete portfolio is packaged below because this upload used the browser. Download `NUS_Dale_Capstone.zip` and extract it. To run the supplied deployment model without retraining, copy `capstone_part3/models/forest_regression_v1.joblib` to `capstone_part3/models/deployment.joblib`. The ZIP omits duplicate MLflow artifact copies; its SQLite store retains the experiment metrics.

The full repository, including the actual five incremental development commits and all MLflow artifacts, is in `history.part1` and `history.part2`. Restore it with:

```bash
cat history.part1 history.part2 > capstone.bundle
git clone capstone.bundle NUS_Dale_Capstone
```

On Windows Command Prompt: `copy /b history.part1+history.part2 capstone.bundle`, then the same git clone command. The web repository upload commit is packaging; the five development commits are preserved in the bundle.
